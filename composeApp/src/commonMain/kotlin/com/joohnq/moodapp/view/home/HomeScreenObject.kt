...
fun HomeScreenUi(
    ...
    healthMetricsItems: List<MentalMetric>
) {
    if (user == null || recentMood == null) return
...
fun HomeScreen(
    ...
    val monthlyMoods = moodsViewModel.monthlyMoods.collectAsState().value.getValueOrNull() ?: return
    val recentMood by remember { mutableStateOf<StatsRecord?>(null) }
    ...
    HomeScreenUi(
        user = user.getValueOrNull(),
        recentMood = recentMood,
        healthMetricsItems = healthMetricsItems
    )
}
...
fun HomeScreenPreview() {
    HomeScreenUi(
        ...
        healthMetricsItems = remember {
            listOf(
                MentalMetric.FreudScore { modifier ->
                    FreudScoreComponent(
                        modifier,
                        FreudScore.AtRisk(60)
                    )
                },
                MentalMetric.Mood { modifier ->
                    MoodComponent(
