...
fun <R1, R2, R3> List<UiState<*>>.fold(
    onLoading: () -> Unit = {},
    onError: (Throwable) -> Unit = {},
    onSuccess: (R1, R2, R3) -> Unit,
) {
    if (any { it is UiState.Error }) {
        val errorState = first { it is UiState.Error } as UiState.Error
        onError(errorState.error)
        ...
        onSuccess(r1.data, r2.data, r3.data)
    } else {
        onLoading()
    }
}

data class TripleState<R1, R2, R3>(
    val first: UiState<R1>,
    val second: UiState<R2>,
    val third: UiState<R3>,
)

inline fun <R1, R2, R3, T> TripleState<R1, R2, R3>.whenAllResolved(
    onLoading: () -> T,
    onError: (Throwable) -> T,
    onSuccess: (R1, R2, R3) -> T,
): T {
    return when {
        first is UiState.Error -> onError(first.error)
        second is UiState.Error -> onError(second.error)
        third is UiState.Error -> onError(third.error)

        first is UiState.Success && second is UiState.Success && third is UiState.Success -> {
            onSuccess(
                (first as UiState.Success<R1>).data,
                (second as UiState.Success<R2>).data,
                (third as UiState.Success<R3>).data
            )
        }
...
fun <R1, R2, R3, R4> List<UiState<*>>.fold(
    onLoading: () -> Unit = {},
    onError: (Throwable) -> Unit = {},
    onSuccess: (R1, R2, R3, R4) -> Unit,
) {
    if (any { it is UiState.Error }) {
        val errorState = first { it is UiState.Error } as UiState.Error
        onError(errorState.error)
        ...
        onSuccess(r1.data, r2.data, r3.data, r4.data)
    } else {
        onLoading()
    }
}
...
@Composable
fun <R1, R2, R3> List<UiState<*>>.handle(
    onLoading: @Composable () -> Unit = {},
    onError: @Composable (Throwable) -> Unit = {},
    onSuccess: @Composable (R1, R2, R3) -> Unit,
) {
    if (any { it is UiState.Error }) {
        val errorState = first { it is UiState.Error } as UiState.Error
        onError(errorState.error)
...
@Composable
fun <R1, R2, R3, R4> List<UiState<*>>.handle(
    onLoading: @Composable () -> Unit = {},
    onError: @Composable (Throwable) -> Unit = {},
    onSuccess: @Composable (R1, R2, R3, R4) -> Unit,
) {
    if (any { it is UiState.Error }) {
        val errorState = first { it is UiState.Error } as UiState.Error
        onError(errorState.error)
...
@Composable
fun <R1, R2, R3, R4, R5> List<UiState<*>>.handle(
    onLoading: @Composable () -> Unit = {},
    onError: @Composable (Throwable) -> Unit = {},
